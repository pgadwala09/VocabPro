...
export const LearningInsightsCard = ({ feedback }) => {
  if (!feedback) return null;
  return (
    <div className="bg-gradient-to-br from-blue-900 to-purple-800 rounded-2xl shadow-xl p-8 w-full max-w-md mx-auto mb-8 text-white relative flex flex-col items-center">
      <h2 className="text-2xl font-bold mb-4 text-center">Learning Insights</h2>
      <div className="flex flex-col items-center mb-6">
        <div className="w-28 h-28 rounded-full bg-gradient-to-br from-purple-400 to-blue-400 flex items-center justify-center text-5xl font-bold border-8 border-blue-200 mb-2">
          {feedback.score}
        </div>
      ...
      <div className="w-full space-y-4 mb-4">
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            ...
            <div className="text-blue-200 text-sm">Did I understand the definition?</div>
          </div>
          <div className="flex-1 flex flex-col items-end">
            <div className="w-32 h-2 bg-blue-200/30 rounded-full overflow-hidden mb-1">
              <div className="h-2 bg-blue-400 rounded-full" style={{ width: `${(feedback.meaning?.value || 0) * 10}%` }}></div>
            </div>
            <div className="text-xs text-blue-100">{feedback.meaning?.text || '-'}</div>
          </div>
        </div>
        <div className="flex items-center gap-3">
          ...
          <div className="flex-1">
            ...
            <div className="text-blue-200 text-sm">Was I using the word correctly in context?</div>
          </div>
          <div className="flex-1 flex flex-col items-end">
            <div className="w-32 h-2 bg-blue-200/30 rounded-full overflow-hidden mb-1">
              <div className="h-2 bg-blue-400 rounded-full" style={{ width: `${(feedback.usage?.value || 0) * 10}%` }}></div>
            </div>
            <div className="text-xs text-blue-100">{feedback.usage?.text || '-'}</div>
          </div>
        </div>
        <div className="flex items-center gap-3">
          <span className="text-2xl">🎤</span>
          <div className="flex-1">
            ...
            <div className="text-blue-200 text-sm">Did I speak clearly?</div>
          </div>
          <div className="flex-1 flex flex-col items-end">
            <div className="w-32 h-2 bg-blue-200/30 rounded-full overflow-hidden mb-1">
              <div className="h-2 bg-blue-400 rounded-full" style={{ width: `${(feedback.clarity?.value || 0) * 10}%` }}></div>
            </div>
            <div className="text-xs text-blue-100">{feedback.clarity?.text || '-'}</div>
          </div>
        </div>
      ...
      <button className="mt-auto px-8 py-3 bg-blue-400 hover:bg-blue-500 text-white font-bold rounded-lg text-lg shadow">Practice Now</button>
    </div>
  );
};
