...
import { fetchWordDefinition, getWordMeaning, getWordExample } from '../lib/dictionaryApi';
...
const SpellingPracticeTrainer: React.FC = () => {
  ...
  // Fetch meaning and example when current word changes in Word Flashcards mode
  useEffect(() => {
    if (mode === 'Word Flashcards' && SPELLING_WORD.word) {
      setLoadingMeaning(true);
      fetchWordDefinition(SPELLING_WORD.word)
        .then((data) => {
          setMeaning(getWordMeaning(data));
          setExample(getWordExample(data));
        })
        .catch(() => {
          setMeaning('Meaning not available');
          setExample('Example not available');
        })
        .finally(() => setLoadingMeaning(false));
    }
  }, [mode, SPELLING_WORD.word]);
    ...
    <div className="min-h-screen bg-gradient-to-br from-purple-900 via-blue-900 to-indigo-900 flex flex-col items-center">
      ...
      <div className="flex flex-row gap-8 w-full max-w-6xl mb-8 mt-8">
        ...
        <div className="flex flex-col w-[700px] h-[540px] bg-gradient-to-br from-blue-800 to-blue-600 border-2 border-white rounded-2xl p-0 relative" style={{boxShadow: '0 4px 40px 0 rgba(0,0,0,0.15)'}}>
          ...
          {/* Practice Area */}
          {mode === 'Word Flashcards' ? (
            <div className="flex flex-col items-center justify-center w-full h-full flex-1 px-4 py-6">
              {/* Navigation */}
              <div className="flex justify-between w-full mb-4">
                <button onClick={() => setCurrent((prev) => (prev - 1 + quizWords.length) % quizWords.length)} className="text-blue-300 font-bold flex items-center gap-2 text-xl px-4 py-2 rounded-lg hover:bg-blue-700/30">Prev</button>
                <span className="text-blue-100 font-semibold">{current + 1} / {quizWords.length}</span>
                <button onClick={() => setCurrent((prev) => (prev + 1) % quizWords.length)} className="text-blue-300 font-bold flex items-center gap-2 text-xl px-4 py-2 rounded-lg hover:bg-blue-700/30">Next</button>
              </div>
              {/* Word Title */}
              <div className="text-5xl font-extrabold text-white mb-8 text-center tracking-wide drop-shadow-lg mt-2">{SPELLING_WORD.word}</div>
              {/* Meaning */}
              <div className="text-2xl text-white mb-6 text-center font-medium drop-shadow">{loadingMeaning ? 'Loading meaning...' : meaning}</div>
              {/* Sample Sentence */}
              <div className="text-2xl font-bold text-white mb-2 text-center">Sample Sentence</div>
              <div className="text-xl italic text-white mb-8 text-center">{loadingMeaning ? 'Loading example...' : example}</div>
              {/* Next button for navigation to word checker */}
              <button className="bg-blue-600 hover:bg-blue-700 text-white font-bold px-8 py-3 rounded-xl text-lg shadow mt-8" onClick={() => navigate('/word-checker')}>Next</button>
            </div>
          ) : (
      ...
      {/* Next Section Button */}
      <button
        className="fixed bottom-8 right-12 bg-blue-600 hover:bg-blue-700 text-white font-bold px-10 py-4 rounded-xl text-lg shadow z-50"
        onClick={() => navigate('/word-checker')}
        style={{ position: 'fixed', bottom: '2rem', right: '3rem', zIndex: 50 }}
      >
        Next
      </button>
    </div>
